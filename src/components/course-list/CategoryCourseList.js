...
const CategoryCourseList = ({catData}) => {
  ...
  const fetchCourses = async pageNum => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/custom-products?page=${pageNum}&per_page=${post_per_page}&sort=default&category=${catData.catId}`,
      );
        ...
        <FlatList
          ListHeaderComponent={
            <View style={styles.categoryHeader}>
              <Text style={styles.categoryHeaderTxt}>
                {decode(catData.catName)} Courses
              </Text>
            </View>
          }
          ListFooterComponent={
